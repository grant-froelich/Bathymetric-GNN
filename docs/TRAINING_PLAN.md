...
**Common confusion**: "Thousands of iterations" does not mean thousands of surveys.

| Term | Meaning |
|------|---------|
| **Survey pair** | One dirty + one clean BAG (ground truth) |
| **Tile** | A subset of the survey (e.g., 1024×1024 cells) extracted for training |
| **Epoch** | One complete pass through ALL training tiles |
| **Iteration** | Processing one batch of tiles within an epoch |

**Example with 5 survey pairs, 20 tiles each, 50 epochs:**
- Total tiles: 5 × 20 = 100 tiles
- Each epoch: model sees all 100 tiles
- Total training passes: 100 tiles × 50 epochs = 5,000 tile presentations

The "thousands of times" refers to total weight updates across epochs, not survey count. The model learns rapidly in early epochs, then plateaus. 50 epochs is a starting point for testing.

## Ground Truth Acquisition Options

| Source | Pros | Cons |
|--------|------|------|
| **Field pairs** | Real-world noise patterns, already in workflow | Requires coordination with field units |
| **Archive recovery** | Large volume available, diverse conditions | Requires NCEI pull + CARIS "Restart cleaning" |
| **Synthetic injection** | Controlled noise types, unlimited volume | May not match real noise patterns exactly |

**Recommended approach**: Combine archive recovery (for volume and diversity) with field pairs (for current noise patterns). Synthetic noise is useful for initial testing but should not be the only training source for production models.

## Why Train on Diverse Data (Not Region-Specific)

The model is designed to generalize across regions rather than being tuned per-region:

| Approach | Advantage | Disadvantage |
|----------|-----------|--------------|
| Region-specific | Optimized for one area | Does not transfer, must retrain for each region |
| Generalized | Works everywhere, learns seafloor *types* | May be slightly less optimal for any single region |

The GNN uses attention heads to learn different seafloor types (flat mud, rocky, etc.) rather than geographic locations. A well-trained model should recognize "this looks like rocky bottom" regardless of whether it's SE Alaska or the Gulf Coast.

**If a specific region consistently underperforms**, targeted training data from that region can be added without losing generalization capability.
