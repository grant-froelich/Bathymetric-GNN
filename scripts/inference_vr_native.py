#!/usr/bin/env python3
"""
Inference script for native VR BAG processing.

Processes VR BAGs without resampling, preserving the multi-resolution structure.
Each refinement grid is processed separately through the GNN.
"""
...
from data import GraphBuilder, VRBagHandler, VRBagWriter
from data.vr_bag import SidecarBuilder
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Native VR BAG inference - preserves multi-resolution structure"
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Output VR BAG file (copy with corrections applied)",
    )
